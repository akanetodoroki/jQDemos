...
function myStaticComboField(id,parent,label,labelwidth,top,left,height,width,items,value,checkbox){
	...
	$("#"+id).jqxComboBox({animationType: 'none', source: eval(id+'source'), multiSelect: false, width: width, height: height, autoDropDownHeight: true});
	if (value!=undefined && value!=''){  //设置初值
...
//生成Tabs tttttttab
function myTabs(id,parent,title,items,top,left,height,width,collapsible){
	...
	if (collapsible!=undefined && collapsible){
		$("#"+id).jqxTabs({ showCloseButtons: false, collapsible: true});
	}
...
function myFormx(id,parent,title,items,top,left,height,width,collapsible){
	var str='<div id=\''+id+'\'>';
	str+='<div>';
...
//生成window窗体，可以关闭
function myWindowx(id,title,top,left,height,width,buttons,resize,drag,close,collapse){
	...
	if (resize!=undefined && resize==1)	str+=', resizable: true';
	else str+=', resizable: false';
	if (drag!=undefined && drag==0) str+=', draggable: false';
	else str+=', draggable: true';
	if (close!=undefined && close==0) str+=', showCloseButton: false';
	if (collapse!=undefined && collapse==1) str+=', showCollapseButton: true';
	str+='});\n';
...
//生成无法关闭的表单
function myForm(id,title,tabs,top,left,height,width,drag,collapse){
	...
	if (drag!=undefined && drag==1) $("#"+pid).jqxWindow({ draggable: true });
	else $("#"+pid).jqxWindow({ draggable: false });
	if (collapse!=undefined && collapse==1) $("#"+pid).jqxWindow({ showCollapseButton: true });
...
//生成窗体modal
function myWindow(id,title,tabs,top,left,height,width,buttons,modal,resize,drag,close,collapse){	//生成myFormWindow//主窗体, myFormPanel-外层fieldset，myForm-里层fieldset
	...
	if (modal!=undefined && modal==1){
		$("#"+pid).jqxWindow({ isModal: true, modalOpacity: 0.1 });
	}
	if (resize!=undefined && resize==1)	$("#"+pid).jqxWindow({ resizable: true });
	else $("#"+pid).jqxWindow({ resizable: false });
	if (drag!=undefined && drag==0) $("#"+pid).jqxWindow({ draggable: false });
	else $("#"+pid).jqxWindow({ draggable: true });
	if (close!=undefined && close==0) $("#"+pid).jqxWindow({ showCloseButton: false });
	if (collapse!=undefined && collapse==1) $("#"+pid).jqxWindow({ showCollapseButton: true });
	if (top!=undefined && left!=undefined && top>0 && left>0){ 
		$("#"+pid).jqxWindow({ position: { y: top, left: left }});
	} 
