...
<%
	...
	*/
	...
	if (sortField.equals("")){
    	querystring="select * from ("+sqlString+") as p order by ancester+"+keyField;
    }else{
    	querystring="select * from ("+sqlString+") as p order by "+sortField;
    }
	...
	if(rowCount>0){//判断结果集是否含有值
		...
		while(node_rs.next()){//循环输出结果集
			...
			str+="{\"id\":\""+StringUtil.filterNull(node_rs.getString(keyField)).trim().replace("\"","").replace("'","\\'")+"\"";  
			for (j=1;j<=xcolcount;j++) {
				field=rsmd.getColumnName(j).toLowerCase();
