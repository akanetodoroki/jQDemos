...
//老师要求看懂这段jsp*******
	...
	try{
		...
		node_rs=null;
		System.out.println("node-rs=null");	
	}
	...
	if (node_rs!=null){ //判断结果集是否含有值
		...
		while(node_rs.next()){
			...
			if (isParentFlag>0){ //下面的closed很重要
				//子节点的值也要各不相同  =下划线+主键值
				String vid="_"+StringUtil.filterNull(node_rs.getString("id")).trim().replace("\"","").replace("'","\\'")+"\"";
				results+=str+",state:\"closed\",\"children\":[{\"id\":\""+vid+",\"cid\":\"\", \"text\":\" \"";  //添加一个虚拟空节点
	...
	System.out.println("childnodes="+results);		
	if (rs!=null) rs.close();
