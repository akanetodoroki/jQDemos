...
<%
	...
	//从数据库取记录，转换成json格式数据
	while(node_rs.next()){
		str="{\"value\":\""+StringUtil.filterNull(node_rs.getString(keyField)).trim().replace("\"","").replace("'","\\'")+"\"";   //这里\之后的"为普通字符
		for (j=1;j<=xcolcount;j++) {
			field=rsmd.getColumnName(j).toLowerCase();
		...
		if (isparentflag==1){
			result.append(str+",\"items\":[{\"value\":\" \", \"label\":\" \"}]");  //保留空格
			result.append("}");
			result.append((i==rowCount-1?"":",")+"");
		}else{
			if (style.indexOf("icon")>=0) result.append(str+",icon: \"system/images/notepad.png\" }");
