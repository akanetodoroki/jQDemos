...
<html>
	...
	<script type="text/javascript">
  	//master可以使树或Grid,detail可以是可编辑表格，有form时显示window
	...
	Ext.onReady(function(){
		...
		if (p.tree!=undefined) p.masterobj='tree';
		else p.masterobj='grid';
		eval(mySetupGrid('myGrid1'));
		...
		if (p.detailgrid!=undefined){
			...
			for (var i=1;i<=pItemGridCount;i++) {
				...
				eval(mySetupDetailGrid('myItemGrid'+i));
			}
		}
		//存储grid中combo之类的sql语句，bbar等
		//alert(pmyTree1.region);
		eval(pmyForm.gridcmpstr);
		eval(myDefineGrid(pmyGrid1));//生成grid
		eval(myDefineTree(pmyTree1)); //定义树myTree1
		if (p.masterobj=='tree') myGrid1.setVisible(false);
		else myTree1.setVisible(false);
		...
		eval(myAddHiddenFields(pmyTree1.fieldset,'myForm1'));
		...
		*/
		...
		if (p.tree!=undefined){
			var myView=Ext.create('Ext.Viewport', {
		        ...
		        defaults: { split: true },
		        items: [tbar,myTree1,myItemGridTab]
		    });			
		}else{
			var myView=Ext.create('Ext.Viewport', {
		        layout: { type: 'border', padding: 5 },
		        defaults: { split: true },
		        items: [tbar,myGrid1,myItemGridTab]
		    });			
		}	
		...
		//myTree1事件
		myTree1.on('xbeforeload',function(store) {  //bbbbbbe
			//var newparams={ database:sysdatabasestring, sqlString:pmyTree1.sql,keyField:'cid' };
			//Ext.apply(store.proxy.extraParams,newparams);
			//store.proxy.url='system//fn_getChildNodes.jsp';					
		});				

		myTree1.on('xload',function(){
		...
		myTree1.on('select',function(model, record, index) {//监听树选中时间
			pmyTree1.pageno=1;
			pmyGrid1.index=0;
			if (record.get('isparentflag')==undefined || record.get('isparentflag')==0) 
			fnLoadRecord(record);			           	
		});
		...
		function fnPrintRecord(){  //ppppppp
			...
			var xsql=" select * from categories";
		...
		function fnLoadItemGrid(xpageno,xflag){ //重新加载grid数据,xflag=1强制加载
			if (pItemGridCount>0){
				if (p.masterobj=='tree') var records=myTree1.getSelectionModel().getSelection();
				else var records=myGrid1.getSelectionModel().getSelection();
		...
		function fnLoadRecord(record){  //lll
			...
			if(record){	
				...
				for (var j=0;pmyForm.labelfielddim!=undefined && j<pmyForm.labelfielddim.length;j++){
					...
					Ext.getCmp(ff).setText(record.get(ff));
				}
				for (var i=1;i<=pItemGridCount;i++) {
