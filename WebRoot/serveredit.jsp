	...
	Ext.onReady(function(){
		...
		var myWindow = Ext.create('Ext.window.Window', {
			...
			buttons: [{
				...
				handler: function () {
    				...
    				//window.location = '/crmlab/index.jsp';
				 	return;							
            	}
			}],
			listeners: {
		...
		function fnDeleteCon(e){
			...
			Ext.Ajax.request({
				...
				form: 'myForm',
				params:{ database: xdatabasestring, action: 'delete' },									
   				method: 'POST',async:false,
				callback:function(options,success,response){
			...
			if (xerrormsg==''){
				...
				Ext.Ajax.request({
					...
					params:{ database: xdatabasestring },									
     				method: 'POST',async:false,
   	    			callback:function(options,success,response){
       					//var xerrormsg=Ext.decode(response.responseText).trim();
       					xerrormsg=response.responseText.trim();
       					if (e.id=='btn1'){ //显示错误信息
	       					if (xerrormsg=='') {
