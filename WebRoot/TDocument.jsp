...
<html>
	...
	Ext.onReady(function(){
		...
		pmyTree2.sysdatabasestring=sysdatabasestring; //传递到fn_function.js
		...
		if (p.tree!=undefined) pmyTree2=myGetTreeAttrs(p.tree[0],pmyTree2);
		...
		//mytree2
		pmyTree2.height=0;
		if (pmyTree2.width==0) pmyTree2.width=200;
		pmyTree2.roottitle='全部'+pmyGrid1.tabletitle;
		if (pmyTree2.nodefields!=undefined && pmyTree2.nodefields!=''){
			pmyTree2.groupfields=pmyTree2.nodefields+";-;*/取消分组";
		}else{
			pmyTree2.groupfields='';
		}
		pmyTree2.region='center';
		pmyTree2.selectedcode='';
		pmyTree2.rootcode='';
		pmyTree2.searchtext='';
		pmyTree2.filterfield='';
		pmyTree2.events='storeload;treeload';
		pmyTree2.sql=''; 
		pmyTree2.title='已审核';
		pmyTree2.parentvalue='';
		pmyTree2.keyfield='';  //初始设置为空
		pmyTree2.sortfield='';
		pmyTree2.reloadflag=1;
		pmyTree2.singleexpand='true';
		pmyTree2.pageno=0;		
		pmyTree2.index=-1;	
		//pmyTree2.tbar='';//treetbar
		//pmyTree2.bbar='';
		//pmyTree2.fields='cid;text;nodetext;level;sysnumber;parentnodeid';
		pmyTree2.fields='';  //否则按表格显示
		pmyTree2.name='myTree2';		
		pmyTree2.varname='pmyTree2';		
		pmyTree2.groupfield=[];	
		if (pmyTable.document.datefield!=''){	
			pmyTree2.groupid=''; //pmyTable.document.datefield;
			pmyTree2.grouptext=pmyGrid1.tabletitle+'日期';
		}else{
			pmyTree2.groupid='';
			pmyTree2.grouptext=pmyGrid1.tabletitle;
		}		
		//生成编译得到的控件和事件		
	    ...
	    items:[
	    	...
	    	{xtype:'menuseparator',id:'poptitlebar1'},
		...
		var wintoolbar=Ext.create('Ext.toolbar.Toolbar',{
			region:'north',split:false,items:[
				{id:'xrowfirst',iconCls: 'rowfirstIcon',text:'',width:22, handler: function(e) { fnMoveRecord(e.id,pmyGrid1); },tooltip: '第一行'},
				{id:'xrowprev',iconCls: 'rowprevIcon',text:'',width:24, handler: function(e)   { fnMoveRecord(e.id,pmyGrid1); },tooltip: '上一行'},
				{id:'xrownext',iconCls: 'rownextIcon',text:'',width:24, handler: function(e)   { fnMoveRecord(e.id,pmyGrid1); },tooltip: '下一行'},
				{id:'xrowlast',iconCls: 'rowlastIcon',text:'',width:24, handler: function(e)   { fnMoveRecord(e.id,pmyGrid1); },tooltip: '最后行'}
			]
		});
		...
		var root2=myTree2.setRootNode({
			text:pmyTree2.roottitle,
			iconCls: 'folderopenIcon',
            cid:'',
            level:0,
            expanded:false,                
		...
		if (pmyTable.document.reviewflag==''){
			...
			//}
			eval(myDefineTab('myTreeTab',pmyTree1.bartitle,'west','[myTree1,myTree2]',pmyTree1.height,pmyTree1.width,'true',''));
		}       
		//定义页面布局
		if (pmyForm.modal!='window'){
			...
			var myView=Ext.create('Ext.Viewport', {
		        ...
		        defaults: { split: true },
		        items: [tbar,myTreeTab,Ext.create('Ext.panel.Panel',{
		        	region:'center',
		        	layout:'border',
		        	items:[
		        		myBtnPanel,
		...
		if (pmyTable.document.reviewflag!=''){
  			pmyTable.document.unreviewed=" ("+pmyTable.document.reviewflag+"='' or "+pmyTable.document.reviewflag+" is null)";
  			pmyTable.document.reviewed=" ("+pmyTable.document.reviewflag+"!='' and "+pmyTable.document.reviewflag+" is not null)";
  		}
		//myTree1事件
		...
		myTree1.on('containercontextmenu',function(tree, e){  //定义右键   		
   		...
   		//mytree2
		myTree2.on('beforeload',function(store) {
			var sql=fnGenTreeSql(myTree2);
			var newparams={ database:sysdatabasestring, sqlString:sql,keyField:'cid' };
			Ext.apply(store.proxy.extraParams,newparams);
			store.proxy.url='system//fn_getChildNodes.jsp';					
		});				
		myTree2.on('load',function(){
	    	var root=myTree2.getRootNode();
	    	root.set('text',pmyTree2.grouptext+'（'+root.childNodes.length+'组）');
	    	root.expand();
			var records=this.getSelectionModel().getSelection();
			if (records[0] && records[0]!=root){
			   	records[0].set('text',records[0].get('text')+'（'+records[0].childNodes.length+'组）');
		...
		myTree2.on('select',function(model, record, index) {//监听树选中时间
			pmyTree2.pageno=1;
			pmyGrid1.index=0;			
           	if (pmyForm.modal!='window') fnSetBtn(1);
			fnLoadmyGrid1(1,'','tree');
		});
		myTree2.on('containercontextmenu',function(tree, e){  //定义右键   		
   			e.preventDefault();
   			myContextMenu2.showAt(e.getXY());
   		});
   		myTree2.on('itemcontextmenu',function(tree, record, item, index, e){  //定义右键   		
   			e.preventDefault();
   			myContextMenu2.showAt(e.getXY());
   		});
		...
		*/
		...
		myGrid1.on('select',function(model,record,index) {  //schange
			...
			if (pmyTable.document.reviewflag!='' && myTreeTab.getXType()=='tabpanel') var tabno=myGetActiveTabIndex(myTreeTab);
			else var tabno=1;
		...
		function fnAddRecord(){//新增 aaaaaa
			...
			//增加一条明细表空记录
			if (pmyTable.document.reviewflag!='' && myTreeTab.getXType()=='tabpanel') var tabno=myGetActiveTabIndex(myItemGridTab);
			else var tabno=1;
		...
		function fnReviewRecord(){  //rrrrr审核
			...
			for (var j=0;j<records.length;j++){
				if (records[j].get(pmyTable.document.reviewflag)==''){
					records[j].set(pmyTable.document.reviewflag,sysusername);
					if (Ext.getCmp(pmyTable.document.reviewflag)!=null){
						Ext.getCmp(pmyTable.document.reviewflag).setValue(sysusername);
					}
					var flag=true;
				}else{
					records[j].set(pmyTable.document.reviewflag,'');
					if (Ext.getCmp(pmyTable.document.reviewflag)!=null){
		...
		function fnSaveRecord(){   //ssssss
			...
			*/
			...
			if (xaddoredit=='3'){ //保存审核结果
				...
				var records=myGrid1.getSelectionModel().getSelection();//获取gird所有选中的records
				for (var j=0;j<records.length;j++){
					updatesql+=" update "+pmyGrid1.tablename+" set "+pmyTable.document.reviewflag+"='"+Ext.getCmp(pmyTable.document.reviewflag).getValue()+"' where "+pmyGrid1.keyfield+"='"+records[j].get(pmyGrid1.keyfield)+"' \n";
				}
				...
				if (xaddoredit=='1' && kflag>=0) pmyTable.error+=myPKUniqueCheck(pmyTable);
				//后台验证主码值是否重复
		...
		function fnDeleteRecord(){ //删除客户 dddddde
			var records=myGrid1.getSelectionModel().getSelection();
			if (!records[0]){
				...
				var querysql='';
				if (pmyTable.document.reviewflag!=''){
					sql+=" and ("+pmyTable.document.reviewflag+"='' or "+pmyTable.document.reviewflag+" is null)";
  					querysql+="select top 1 "+pmyTable.document.reviewflag+" as 'reviewflag' from "+pmyGrid1.tablename+" where "+pmyGrid1.keyfield+"='"+xid+"' ";
					querysql+=" and ("+pmyTable.document.reviewflag+"!='' and "+pmyTable.document.reviewflag+" is not null)";
				}else{
		        	querysql+=+"select '' as 'reviewflag'";
				} 					
				//删除明细表
	        	for (var j=1;j<=pItemGridCount;j++){
					var pgrid=eval("pmyItemGrid"+j);
					var grid=eval("myItemGrid"+j);
					var mid=records[0].get(pgrid.masterfield);
					sql+=" delete "+pgrid.tablename;
					//删除时判断审核状态。已经他人审核的不能删除
					if (pmyTable.document.reviewflag!=''){
						sql+=" where "+pgrid.masterfield+" in (select "+pgrid.masterfield+" from "+pmyGrid1.tablename+" where "+pgrid.masterfield+"='"+mid+"'"; 
						sql+=" and ("+pmyTable.document.reviewflag+"='' or "+pmyTable.document.reviewflag+" is null))\n";
					}else{
						sql+=" where "+pgrid.masterfield+"='"+mid+"'\n"; 
					}
				}
				//console.log(sql);	
				Ext.MessageBox.show({   
		            ...
		            fn: function(btn){
		            	if(btn=='yes'){
				            Ext.Ajax.request({
					        	url:'system/fn_executeSql.jsp',
								params:{ database:sysdatabasestring, updateSql:sql,selectSql:querysql },									
					        	method: 'POST',async:false,
					        	callback:function(options,success,response){
	        						var reviewflag=Ext.decode(response.responseText).reviewflag;
									pmyTable.error=Ext.decode(response.responseText).errors;
									if (reviewflag!=undefined && reviewflag!=''){
										pmyTable.error+="<br>"+pmyGrid1.tabletitle+"<b>["+xid+"]</b>已由<b>"+reviewflag+"</b>审核，无法删除！";
									}
									if (pmyTable.error==''){
						        		//删除记录后的定位处理
										...
										eval(sysError('发现下列错误，'+pmyGrid1.tabletitle+'删除失败！<br>'+pmyTable.error,0,0));									
									}//if error		
					        	}//callback
				           });  //ajax	            	
		            	}//if yn
		...
		function fnRefreshRecord(){  //rrrrre刷新
			...
			pmyTree2.reloadflag=1;
			if (pmyTable.document.reviewflag!='' && myTreeTab.getXType()=='tabpanel') myTreeTab.setActiveTab(0);
			pmyTree1.pageno=1;
			pmyTree2.pageno=1;
			pmyGrid1.index=0;
		...
		function fnLoadmyGrid1(pageno,wheresql,flag){ //lllllo重新加载grid数据
			...
			if (pmyTable.document.reviewflag!='' && myTreeTab.getXType()=='tabpanel'){
				var tabno=myGetActiveTabIndex(myTreeTab);
				if (tabno==1) var wheresqlx=pmyTable.document.unreviewed;
				else var wheresqlx=pmyTable.document.reviewed;
			}else{
				var tabno=1;
				var wheresqlx='';
			}
			...
			if (pItemGridCount>0){
				...
				if (records[0]){
					...
					for(var i=0;i<myItemGridTab.items.items.length;i++){
	       			...
	       			if (mflag=='editable' && pmyTable.editflag[1]+pmyTable.editflag[2]+pmyTable.editflag[3]>0){
	       				mySetCmpVisible('poptitleitem;poptitlebar1;popinsertitem;popappenditem;popdeleteitem;popsaveitem;popresetitem;savebar3;resetbar1',true);
	       			}else{
	       				mySetCmpVisible('poptitleitem;poptitlebar1;popinsertitem;popappenditem;popdeleteitem;popsaveitem;popresetitem;savebar3;resetbar1',false);
	       			}
					...
					if (rflag==1 || xflag==1) {
						...
						//pmyItemGrid.sql=sql;  //不能加
						pmyItemGrid.paramsql=sql;   //分页行数变化时用到
						//console.log(sql);
						var newparams={ database:sysdatabasestring, sqlString:sql,keyField:xkeyfield,sortField:xsortfield,limit:xpagesize,summaryFields:pmyItemGrid.summaryfieldsql };
		...
		function fnmyGroupMenuClick(item,e){
			...
			//alert(tabno+'---'+pmyTree.groupid+'---'+myTreeTab.getXType());
			fnLoadmyTree(myTree);
		}
		function fnLoadmyTree(myTree){   //llllllo
			...
			if (pmyTree.groupid=='*'){
				...
				myTreeTab.collapse(Ext.Component.DIRECTION_LEFT);					
			}else{
				if (pmyTable.document.reviewflag!='' && myTreeTab.getXType()=='tabpanel'){
					myTreeTab.expand(Ext.Component.DIRECTION_LEFT);
				}else{
				...
				/*
					if (tmp.length<2){
						pmyTree.sql=" select "+pmyTree.groupfield[1]+" as nodeid,"+"cast("+pmyTree.groupfield[1]+" as varchar(255))+case when "+pmyTree.groupfield[1]+"='' then '其他' else '' end+'('+cast(count(*) as varchar(10))+')' as nodetext,\n";
						pmyTree.sql+="1 as level,0 as isparentflag,'' as parentnodeid,\n";
						pmyTree.sql+="case when "+pmyTree.groupfield[1]+"<>'' then 0 else 1 end as sortflag \n";
						pmyTree.sql+=" from ("+pmyGrid1.sql+") as p \n";
						if (pmyTable.document.reviewflag!=''){					
							if (tabno==1){
								pmyTree.sql+=" where ("+pmyTable.document.reviewflag+"='' or "+pmyTable.document.reviewflag+" is null) and "+pmyTable.document.daterange+"\n";
							}else{
								pmyTree.sql+=" where ("+pmyTable.document.reviewflag+"<>'' and "+pmyTable.document.reviewflag+" is not null) and "+pmyTable.document.daterange+"\n";
							}
						}
						pmyTree.sql+=" group by "+pmyTree.groupfield[1]+"\n";
					}else{  //最多支持两层
						pmyTree.sql=" select cast("+pmyTree.groupfield[1]+" as varchar(255)) as 'nodeid',"+"cast("+pmyTree.groupfield[1]+" as varchar(255))+case when "+pmyTree.groupfield[1]+"='' then '其他' else '' end+'('+cast(count(*) as varchar(10))+')' as nodetext,\n";
						pmyTree.sql+="1 as level,1 as isparentflag,'' as parentnodeid,\n";
						pmyTree.sql+="case when "+pmyTree.groupfield[1]+"<>'' then 0 else 1 end as sortflag \n";
						pmyTree.sql+=" from \n ("+pmyGrid1.sql+") as p \n";					
						if (pmyTable.document.reviewflag!=''){					
							if (tabno==1){
						...
						pmyTree.sql+=" group by "+pmyTree.groupfield[1]+"\n";
						pmyTree.sql+=" union all \n";
						pmyTree.sql+=" select cast("+pmyTree.groupfield[2]+" as varchar(255)) as 'nodeid',cast("+pmyTree.groupfield[2]+" as varchar(255))+case when "+pmyTree.groupfield[2]+"='' then '其他' else '' end+'('+cast(count(*) as varchar(10))+')' as nodetext,";
						pmyTree.sql+="2 as level,0 as isparentflag,"+pmyTree.groupfield[1]+" as parentnodeid,";
						pmyTree.sql+="case when "+pmyTree.groupfield[2]+"<>'' then 0 else 1 end as sortflag ";
						pmyTree.sql+=" from ("+pmyGrid1.sql+") as p ";
						if (pmyTable.document.reviewflag!=''){					
							if (tabno==1){
								pmyTree.sql+=" where ("+pmyTable.document.reviewflag+"='' or "+pmyTable.document.reviewflag+" is null) and "+pmyTable.document.daterange+"\n";
						...
						pmyTree.sql+=" group by "+pmyTree.groupfield[1]+','+pmyTree.groupfield[2];
					}
					*/					
		...
		function fnGenTreeSql(myTree){
			...
			//取各层条件
			var pnode=records[0];
			if (myTree=myTree1 && pmyTable.document.reviewflag!='' && tabno==1) var wheresqly=pmyTable.document.unreviewed;
			else var wheresqly=pmyTable.document.reviewed;
			for (var j=xlevel;j>=1;j--){
		...
		Ext.getCmp('sys_checkedimage').setVisible(false);
		if (pmyTable.document.reviewflag!=''){
			//myTreeTab.setActiveTab(1);
			//fnmyGroupMenuxClick(myGroupMenu[0],null);  //加载树		
			//myTreeTab.setActiveTab(0);
		}
		fnmyGroupMenuxClick(myGroupMenu[0],null);  //加载树		
		//fnLoadmyGrid1(1,'','');
		pmyTree1.reloadflag=0;
